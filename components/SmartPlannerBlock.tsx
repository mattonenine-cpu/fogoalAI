...
export const SmartBlock: React.FC<SmartBlockProps> = ({ 
  ...
  return (
    <div
      className={`absolute w-[95%] left-[2.5%] rounded-xl border shadow-sm group transition-all overflow-hidden select-none backdrop-blur-md ${baseColorClass} ${isDone ? 'opacity-50 grayscale' : ''} ${isMoving ? 'ring-4 ring-indigo-500/60 shadow-[0_0_20px_rgba(99,102,241,0.5)]' : ''}`}
      style={{
        ...style,
        height: `${height}px`,
    ...
    >
      {/* Move Button - Inside top-left corner */}
      <button
        onClick={(e) => {
          e.stopPropagation();
          onStartMove(task);
        }}
        className={`absolute top-1 left-1 w-6 h-6 rounded-md bg-indigo-500 hover:bg-indigo-600 flex items-center justify-center transition-all active:scale-95 shadow-md z-50 ${isMoving ? 'animate-pulse bg-indigo-600 ring-1 ring-indigo-300' : ''}`}
      ...
      {/* Moving indicator */}
      {isMoving && (
        <div className="absolute top-0 left-0 right-0 h-5 flex items-center justify-center bg-indigo-500/20 border-b border-indigo-500/30 z-40">
      ...
      {/* Header */}
      <div 
        className={`flex items-center justify-between px-2 border-b border-black/5 dark:border-white/10 bg-black/5 dark:bg-white/5 ${isMoving ? 'pt-6' : 'h-6'}`}
      >
      </div>

      {/* Content */}
      <div className={`p-2 flex flex-col relative ${isMoving ? 'h-[calc(100%-28px)]' : 'h-[calc(100%-16px)]'}`}>
