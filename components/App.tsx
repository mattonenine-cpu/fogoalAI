...
const [theme, setTheme] = useState<AppTheme>(() => {
  ...
  return (
    <div className="min-h-screen text-[var(--text-primary)] overflow-hidden font-sans transition-colors duration-500 flex flex-col">
      <div className="w-full sm:max-w-md mx-auto min-h-[100dvh] flex flex-col relative z-10 overflow-hidden">
        <header className="p-3 sm:p-5 pb-2 flex justify-between items-center z-40 relative shrink-0 min-h-[52px]">
           ...
           <div className="flex items-center gap-2 shrink-0">
             ...
             <button 
                ...
                className="w-10 h-10 rounded-full glass-liquid flex items-center justify-center text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-all active:scale-90 shrink-0 overflow-visible"
             >
                <SlidersHorizontal size={18} strokeWidth={2} />
             </button>
             <button onClick={handleLanguageCycle} className="px-3 py-1.5 rounded-full glass-liquid text-mini font-bold tracking-widest text-[var(--text-secondary)] hover:text-[var(--text-primary)] transition-colors shrink-0 min-w-[44px]">
               {language.toUpperCase()}
             </button>
           </div>
        </header>

        <main className="flex-1 overflow-y-auto overflow-x-hidden px-3 py-2 scrollbar-hide pb-[120px] min-h-0">
          {renderView()}
        </main>

        <nav className="fixed bottom-6 left-0 right-0 z-[600] flex justify-center px-4 pointer-events-none">
          <div className="glass-liquid rounded-[32px] px-2 py-2.5 flex items-center justify-center gap-1 sm:gap-2 shadow-2xl w-full max-w-md pointer-events-auto bg-[var(--bg-card)] overflow-visible">
...
const NavBtn: React.FC<{ active: boolean, onClick: () => void, emoji: string }> = ({ active, onClick, emoji }) => (
  <button onClick={onClick} className={`w-9 h-9 sm:w-12 sm:h-12 shrink-0 rounded-[14px] sm:rounded-[22px] flex items-center justify-center transition-all duration-300 relative overflow-visible ${active ? 'bg-[var(--bg-active)] text-[var(--bg-active-text)] shadow-lg scale-110' : 'grayscale opacity-60 hover:grayscale-0 hover:opacity-100 hover:bg-white/5'}`}>
    <span className="text-sm sm:text-xl leading-none flex items-center justify-center">{emoji}</span>
    {active && <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-[var(--theme-accent)] rounded-full shadow-[0_0_8px_var(--theme-accent)]" />}
  </button>
);
