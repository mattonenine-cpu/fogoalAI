...
import { EcosystemType, UserProfile, Task, Language, TRANSLATIONS, Practice, AppView, AppTheme } from '../types';
...
export const EcosystemView: React.FC<EcosystemViewProps> = ({ type, user, tasks, lang, onUpdateTasks, onUpdateProfile, onNavigate, theme, onDeductCredits, onLogout }) => {
  ...
  // Локальное состояние маленького обучения по экосистеме
  const [showLocalEducation, setShowLocalEducation] = useState(() => {
    if (lang !== 'ru') return false;
    return !user.ecosystemEducation?.[type];
  });
  ...
  if (type === 'study') {
    return (
      <div className="animate-fadeIn pb-32">
        {lang === 'ru' && showLocalEducation && (
          <div className="mb-4 px-1">
            <div className="rounded-[28px] bg-[var(--bg-card)] border border-[var(--border-glass)] p-4 flex gap-3 items-start shadow-lg">
              <div className="w-8 h-8 rounded-2xl bg-indigo-500/10 flex items-center justify-center text-indigo-400 shrink-0">
                <Lightbulb size={18} />
              </div>
              <div className="flex-1">
                <p className="text-[11px] font-black uppercase tracking-[0.18em] text-[var(--text-secondary)] mb-1">
                  {t.ecoStudyEduTitle}
                </p>
                ...
                <button
                  type="button"
                  onClick={() => {
                    setShowLocalEducation(false);
                    onUpdateProfile({
                      ...user,
                      ecosystemEducation: {
                        ...(user.ecosystemEducation || {}),
                        study: true,
                      },
                    });
                  }}
                  className="mt-3 inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.18em] text-[var(--theme-accent)]"
                >
                  Понятно
                </button>
              </div>
  ...
  if (type === 'sport') {
      return (
        <div className="animate-fadeIn pb-32">
            <header className="flex justify-between items-center px-1 mb-4">
                <div>
            ...
            {lang === 'ru' && showLocalEducation && (
              <div className="px-1 mb-4">
                <div className="rounded-[28px] bg-[var(--bg-card)] border border-[var(--border-glass)] p-4 flex gap-3 items-start shadow-lg">
                  <div className="w-8 h-8 rounded-2xl bg-orange-500/10 flex items-center justify-center text-orange-400 shrink-0">
                    <Lightbulb size={18} />
                  </div>
                  <div className="flex-1">
                    <p className="text-[11px] font-black uppercase tracking-[0.18em] text-[var(--text-secondary)] mb-1">
                    ...
                    <button
                      ...
                      className="mt-3 inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.18em] text-orange-500"
                    >
                      Понятно
                    </button>
                  </div>
      ...
      return (
        <div className="animate-fadeIn pb-32">
            {lang === 'ru' && showLocalEducation && (
              <div className="px-1 mb-4">
                <div className="rounded-[28px] bg-[var(--bg-card)] border border-[var(--border-glass)] p-4 flex gap-3 items-start shadow-lg">
                  <div className="w-8 h-8 rounded-2xl bg-emerald-500/10 flex items-center justify-center text-emerald-400 shrink-0">
                    <Heart size={18} />
                  </div>
                  <div className="flex-1">
                    <p className="text-[11px] font-black uppercase tracking-[0.18em] text-[var(--text-secondary)] mb-1">
                    ...
                    <button
                      ...
                      className="mt-3 inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-[0.18em] text-emerald-500"
                    >
                      Понятно
                    </button>
                  </div>
    ...
    <div className="space-y-6 animate-fadeIn pb-6">
      ...
      {lang === 'ru' && showLocalEducation && (
        <section className="animate-fade-in-up">
          <GlassCard className="p-4 bg-[var(--bg-card)] border-[var(--border-glass)] rounded-[28px] flex gap-3 items-start shadow-lg">
            <div className="w-8 h-8 rounded-2xl bg-indigo-500/10 flex items-center justify-center text-indigo-400 shrink-0">
              <Lightbulb size={18} />
            </div>
            <div className="flex-1">
